  ...
  char* asString(char* buf, char const* format = "%d-%M-%Y") const
  {
    ...
    while ((ch = *format++) != '\0')
    {
      if (ch == '%')
      {
        ...
        case 'd':
          dst += sprintf(dst, "%02u", day );
          continue;

        case 'D':
          dst += sprintf(dst, "%s",   dayName[jday % 7]);
          continue;

        case 'm':
          dst += sprintf(dst, "%02u", month);
          continue;

        case 'M':
          dst += sprintf(dst, "%s",   monthName[month - 1]);
          continue;

        case 'y':
          dst += sprintf(dst, "%02u", year - 1900);
          continue;

        case 'Y':
          dst += sprintf(dst, "%04u", year);
          continue;
